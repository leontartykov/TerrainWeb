...
TEST(tests_vector_3d_get_x, positive)
{
    int x = 1;
    Vector3D<int> vector(x, 0, 0);
    int result = vector.get_x();
    EXPECT_EQ(x, result);
}

TEST(tests_vector_3d_get_y, positive)
{
    int y = 1;
    Vector3D<int> vector(0, y, 0);
    int result = vector.get_y();
    EXPECT_EQ(y, result);
}

TEST(tests_vector_3d_get_z, positive)
{
    int z = 1;
    Vector3D<int> vector(0, 0, z);
    int result = vector.get_z();
    EXPECT_EQ(z, result);
}
